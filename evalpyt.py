...
Options:
    ...
    --predictPath=<str>         Predict masks path [default: result/msra/]
"""
...
predict_path = args['--predictPath']
img_list = open('data/list/valid_msra.txt').readlines()

for iter in range(1,2):   #TODO set the (different iteration)models that you want to evaluate on. Models are saved during training after every 1000 iters by default.
    ...
    for i in img_list:
        ...
        cv2.imwrite(os.path.join(predict_path, i[:-1]+'.png'), output)
        if args['--visualize']:
            plt.subplot(3, 1, 1)
